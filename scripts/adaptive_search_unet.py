#!/usr/bin/env python3
"""Adaptive UNet3D hyperparameter search using Optuna's TPE sampler.

Results accumulate in a SQLite study (resumable). Each trial trains the
unet3d experiment via scripts/train.py and reports the best validation MAE.

Usage:
    python scripts/adaptive_search_unet.py --device cuda --n-trials 50
    python scripts/adaptive_search_unet.py --device cuda --n-trials 100  # resume / extend
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Adaptive UNet3D hyperparameter search via Optuna")
    p.add_argument(
        "--n-trials",
        type=int,
        default=50,
    ...
    p.add_argument("--store-prefix", default="data/models/adaptive_search_unet")
    p.add_argument(
        "--db-path", default=None, help="SQLite DB path (default: <store-prefix>/optuna.db)"
    )
    p.add_argument("--study-name", default="unet3d_adaptive_search")
    p.add_argument(
        "--timeout",
        type=float,
...
def build_train_command(hp: dict, exp_name: str, store_prefix: str, device: str) -> list[str]:
    return [
        ...
        f"network.base_channels={hp['base_channels']}",
        f"exp_name={exp_name}",
...
def make_objective(store_prefix: str, device: str):
    def objective(trial: optuna.Trial) -> float:
        ...
        exp_name = f"unet_t{trial.number:03d}"
